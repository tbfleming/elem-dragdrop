...
cpdefine("inline:com-chilipeppr-elem-dragdrop", ["chilipeppr_ready"], function () {
    ...
    return {
        ...
        getType: function(filename, mime) {
            if (filename.toLowerCase().endsWith('.stl'))
                return 'stl';
            else if (mime.startsWith('image/'))
                return mime;
            else
                return undefined;   // Use non-typed ondropped message
        },
        loadUrlDoNotCreateRecentFileEntry: function(evt) {
            ...
            $.get(url, null, function(data, status, xhr) {
                ...
                var type = that.getType(url, xhr.getResponseHeader('content-type'));
                var info = {
                    name:url, 
                    lastModified: new Date()
                };
                console.log("info:", info);
                if (type)
                    chilipeppr.publish("/com-chilipeppr-elem-dragdrop/ondroppedTyped", type, data, info);
                else {
                    // unsubscribe so we don't get loopback
                    chilipeppr.unsubscribe("/com-chilipeppr-elem-dragdrop/ondropped", that.createRecentFileEntry);
                    // send event off as if the file was drag/dropped
                ...
                drop: function (e) {
                    ...
                    for (var i = 0; i < files.length; i++) {
                        (function (i) {
                            ...
                            //console.log(files[i]);
                            var thefile = files[i];
                            var type = that.getType(thefile.name, thefile.type);
